...
export default function ReviewSubmit({ formData, updateFormData, prevStep, gameId }: ReviewSubmitProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        try {
          ...
          if (file instanceof Blob) {
            if (file instanceof File) {
              const uploadFunction = fileType === "image" ? startImageUpload : startPdfUpload
              uploadResult = await uploadFunction([file])
            } else {
      ...
      submitFormData.append("discordScreenshotUrl", discordScreenshotResult.url)
